...
func encodeByField(u map[string]string, path string, v protoreflect.Message) error {
	for i := 0; i < v.Descriptor().Fields().Len(); i++ {
		...
		case fd.IsMap():
			if v.Get(fd).Map().Len() > 0 {
				...
				for k, value := range m {
					k = newPath + keyDelimiter + strcase.ToCamel(k)
					u[k] = value
				}
			}
